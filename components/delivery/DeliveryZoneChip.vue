<template>
    <button
        ...
        :class="[
            'inline-flex min-h-11 items-center gap-1.5 rounded-xl px-3 py-2 text-xs font-medium transition-all duration-300 ease-out focus:outline-none focus-visible:ring-2 focus-visible:ring-red-300 focus-visible:ring-offset-2 max-w-full',
            stateClasses
        ]"
    >
...
const chipLabel = computed(() => {
    switch (state.value) {
        case 'notSet': return t('delivery.chip.verify')
        case 'inZone': return t('delivery.chip.inZone', { km: distanceKm.value })
        case 'outOfZone': return t('delivery.chip.outOfZone')
        case 'pickup': return t('delivery.chip.pickup')
        default: return ''
    }
})
