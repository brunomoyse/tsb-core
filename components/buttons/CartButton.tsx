...
const CartButton: React.FC = () => {
    ...
    return (
        <button
            className={`flex items-center justify-center rounded-full h-9 w-28 font-semibold text-sm 
                        ${isCartEmpty ? 'bg-tsb-gray text-off-white border-2 border-tsb-red hover:bg-tsb-red hover:text-white' : 'bg-tsb-red-darker'}`}
        >
            ...
            <span>{isCartEmpty ? '' : `• ${totalProductQuantity}`}</span>
            <span>Panier</span>
        </button>
    );
}
