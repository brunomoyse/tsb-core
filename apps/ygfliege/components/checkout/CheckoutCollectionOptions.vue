<template>
    <section class="card p-5 w-full mx-auto space-y-6">
        ...
        <div id="checkout-delivery-address" tabindex="-1" v-if="cartStore.collectionOption === 'DELIVERY'" class="flex flex-col gap-2">
            ...
            <div v-if="cartStore.address" class="flex flex-col text-ygf-black bg-ygf-gray-100 rounded p-3">
                ...
                <button
                    ...
                    class="min-h-11 inline-flex items-center self-start mt-2 px-3 -ml-3 text-sm font-medium text-ygf-orange-600 hover:text-ygf-orange-700 rounded-md focus-visible:ring-2 focus-visible:ring-ygf-orange-300 focus:outline-none transition-colors"
                >
                    {{ $t('checkout.editAddress', 'Edit Address') }}
                </button>
                <p v-if="isExcludedPostcode(cartStore.address.postcode)" class="mt-2 text-sm text-ygf-orange-600 font-medium">
                    {{ $t('checkout.notDeliverableArea') }}
                </p>
                <p v-else-if="cartStore.address.distance >= DELIVERY_ZONE_METERS" class="mt-2 text-sm text-ygf-orange-600 font-medium">
                    {{ $t('checkout.tooFar') }}
                </p>
        ...
        <div class="mt-4">
            ...
            <p v-else-if="!isOpen && availableFixedSlots.length === 0" class="text-ygf-orange-600 font-semibold">
